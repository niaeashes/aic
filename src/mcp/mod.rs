...
//   3. ReplContext に格納し、以降 agent.rs から触る（M7 で配線）
...
impl McpManager {
    ...
    ///
    /// 失敗時の戻り値は `Err`。M7 のエージェントループは Err を `tool` メッセージの
    /// 文面に詰め直してモデルに伝える（呼び出し側の責務）。
    pub async fn call(&mut self, public_name: &str, arguments: Value) -> Result<String> {
        ...
        if parsed.is_error {
            // is_error はサーバが「ツール側論理エラー」と判定したケース。例外で投げ返す。
            anyhow::bail!("MCP tool {public_name} returned error: {text}");
        }
        Ok(text)
    }

    /// `connect_all` 内部で使う登録ヘルパ。テストからも `<server>__<tool>` のカタログ
    /// 命名規則を直接確認できるよう pub(crate)。
    pub(crate) fn push_server(
        ...
        for t in &tools {
            ...
            // 同一 (server, tool) を 2 回登録するのは想定外。後勝ちで上書きする。
...
mod tests {
    ...
    fn dummy_transport() -> Transport {
        // 接続しないので URL は適当でよい。reqwest::Client::new() は副作用なし。
        Transport::new(
    ...
    fn catalog_disambiguates_collisions_across_servers() {
        // 2 つのサーバが同名 `search` を出してきても、`<server>__<tool>` で区別できる。
        ...
        // as_openai_tools は同じ件数を出す
        let tools = mgr.as_openai_tools();
        assert_eq!(tools.len(), 3);
        // OpenAI 形式: type=function、name=公開名
